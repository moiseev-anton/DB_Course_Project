...
class UserService:
    ...
    async def authenticate_user(self, uow: IUnitOfWork, creds: UserLogin) -> UserDisplay:
        async with uow:
            user = await uow.users.get_one(phone=creds.phone, is_active=True)
            if not user or not await self.verify_password(creds.password, user.password_hash):
    ...
    @staticmethod
    async def update_user(uow: IUnitOfWork, user_id: UUID,  user: UserUpdate) -> UserDisplay:
        async with uow:
            ...
            updated_user = await uow.users.update_one(id=user_id, data=user.model_dump(exclude_unset=True))
            await uow.commit()  # Подтверждаем изменения
            return UserDisplay.model_validate(updated_user)

    async def change_password(self, uow: IUnitOfWork, user_id: UUID,  pswd: PasswordChange) -> UserDisplay:
        async with uow:
            ...
            updated_user = await uow.users.update_one(id=user_id, data={'password_hash': password_hash})
